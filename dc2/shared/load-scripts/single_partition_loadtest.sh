...
register_service() {
    ...
    # --- FIX #1: Explicitly declare 'j' as a local variable ---
    local j
    for j in $(seq 1 16); do # Use exactly 64 keys (the max) to get 32KB payload
...
for (( n=1; n<=10; n++ )); do
      ...
      if create_namespace "$partition_name" "$namespace_name"; then
        for (( s=1; s<=10; s++ )); do
          ...
          node_name="node-${service_id}". # limit the total number of nodes to 4/5 - it shouldn't contain the service number
