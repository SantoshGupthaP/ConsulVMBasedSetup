...
# pointing the services to a pool of real workload IPs and including a large
# metadata payload to simulate a ~83KB service footprint.
...
echo "Generating 80KB metadata payload to simulate ALB service size..."
PAYLOAD_SIZE=385
METADATA_PAYLOAD=$(head -c ${PAYLOAD_SIZE} /dev/urandom | base64 -w 0)
...
register_service() {
    ...
    # Use jq to safely construct the JSON payload, avoiding shell expansion issues.
    local SERVICE_JSON
    SERVICE_JSON=$(jq -n \
